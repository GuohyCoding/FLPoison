...
class COMPASS(MPBase, Client):
    ...
    def omniscient(self, clients):
        ...
        if self.fixed_rand is None:
            ...
            self.fixed_rand = self.get_attack_direction_by_weight_importent(pca_direction)
    ...
    def get_attack_direction_by_pca(self, current_global_vec):
        ...
        ortho_vec = v_pca / (v_pca.norm() + 1e-12)
        # 3）归一化得到单位向量
        # ortho_unit = ortho_sign / (ortho_sign.norm() + 1e-12)
        # self._log_message(f"[PCA]{ortho_unit}")
        return ortho_vec.detach().clone()

    def get_attack_direction_by_weight_importent(self, pca_direction):
        best_vec = None
        best_score = -float("inf")
        beta = max(float(self.unimportant_magnitude), 1e-6)
        sign_reference = self._get_auxiliary_sign_reference(pca_direction)
        for n_value in self.n_candidates:
            aux_vec = self._build_auxiliary_vector(
                self.topk_mask, beta, float(n_value), pca_direction, sign_reference
            )
            orthogonal_vec = self.getOrthogonalVectorByRotary(pca_direction, aux_vec)
            orthogonal_norm = orthogonal_vec.norm()
            if orthogonal_norm < 1e-12:
                continue
            attack_vec = orthogonal_vec / orthogonal_norm
            concentration = torch.sum(attack_vec[self.topk_mask] ** 2).item()
            retention = (orthogonal_norm / (aux_vec.norm() + 1e-12)).item()
            sign_agreement = self._compute_sign_agreement(attack_vec, sign_reference)
            score = concentration * retention * sign_agreement
            if score > best_score:
                best_score = score
                best_vec = attack_vec
        if best_vec is None:
            best_vec = self._compute_orthogonal_sign(pca_direction)
            best_vec = best_vec / (best_vec.norm() + 1e-12)
        return best_vec.detach().clone()
        # 1) 求重要权重
        importance_full = self._compute_param_importance()
        # 2) 权重的Top-K mask
