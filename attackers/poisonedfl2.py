...
class PoisonedFL2(MPBase, Client):
    ...
    def omniscient(self, clients):
        ...
        if current_epoch > 50:
            ...
            self.local_mutched_filter_score_history.append(self.local_mutched_filter_score)
    ...
    def adaptive_attack_magnitude(self, sf_min, sf_max, sf_now, slide_window, increse, decrease, threshold):
        ...
        self.local_mutched_filter_score_history.append(float(self.local_mutched_filter_score))
        self.local_accuracy_history.append(float(self.local_accuracy))

        # --- 维护滑动窗口长度（FIFO） ---
        if slide_window > 0:
            while len(self.local_mutched_filter_score_history) > slide_window:
                del self.local_mutched_filter_score_history[0]
            while len(self.local_accuracy_history) > slide_window:
                del self.local_accuracy_history[0]

        # --- 预热期：窗口未满，数据不足 ---
        if slide_window <= 0 or len(self.local_mutched_filter_score_history) < slide_window:
        ...
        # --- 计算分数趋势与准确率趋势 ---
        score_trend = _trend(self.local_mutched_filter_score_history)
        acc_trend = _trend(self.local_accuracy_history)
